...
The variables contains: id, date, prcp, snow, snwd, tmax, tmin. `id`
indicates weather station ID, `date` indicates date of observation,
`prcp` indicates precipitation, `snow` indicates snowfall, `snwd`
indicates snow depth, `tmax` indicates maximun temperature, and `tmin`
indicates minimun temperature.

However, 47.10405% of the observations contain missing value.
